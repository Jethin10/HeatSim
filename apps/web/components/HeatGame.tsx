...
import { TOOL_META, fingerprint, isValidPlacement, makeScenario, optimizeLocal, simulateLocal } from '../lib/simulation';
import type { GameMode, Intervention, InterventionType, OptimizationResult, Overlay, Tool } from '../lib/types';

const BUDGET_LIMIT=3_000_000,WATER_LIMIT=8_000;
...
export default function HeatGame(){
  ...
  const place=(x:number,y:number)=>{
    if(optimizing)return;if(tool==='inspect'){setNotice(`Inspecting cell ${x},${y}. Switch thermal lenses to understand the hotspot.`);return;}
    ...
    setInterventions(old=>[...old,{id:`${tool}-${x}-${y}-${Date.now()}`,type:tool,x,y}]);setPulse({x,y,at:performance.now()});setManualScore(null);setOptimizedScore(null);setNotice(`${meta.label} placed. The field is recomputing spatially.`);
  };
  ...
  const optimize=async()=>{
    ...
    await new Promise(r=>window.setTimeout(r,120));if(runToken.current!==token)return;
...
function PlacementPreview({valid,tool,current,next,land}:{valid:boolean;tool:InterventionType;current:number;next:number|null;land:string}){const meta=TOOL_META[tool];return <div className={`placement-preview ${valid?'valid':'invalid'}`}><span>{valid?'PLACEMENT PREVIEW':'NOT RECOMMENDED'}</span><b>{valid?meta.label:`${meta.label} on ${land}`}</b>{valid&&next!==null?<><div><small>Marginal exposure effect</small><strong>{pct(next-current)}</strong></div><p>{meta.water?`${meta.water} L/day water · `:''}{money(meta.cost)} capital proxy</p></>:<p>This intervention is physically incompatible with the selected land cell.</p>}</div>}
